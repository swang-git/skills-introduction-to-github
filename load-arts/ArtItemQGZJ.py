...
class Art:
    ...
    def is_item_for_theday(self):
        # print(self.dng, self.tim[0:10], self.theday, self.tim)
        return (self.dng and self.tim[0:10] <= self.theday) or self.theday in self.tim
        # return self.theday in self.tim
    ...
    def set_tim(self):
        ...
        if tim is not None:
            try:
                ...
                tdate = tim.group(4)
                thhmm = tim.group(6)
                # if tmnth == '01': self.year = '2020'
                # self.tim = self.year + '-' + tim.group(2) + '-' + tim.group(4) + ' ' + tim.group(6)
                self.tim = self.year + '-' + tmnth + '-' + tdate + ' ' + thhmm
    ...
    def get_txt(self):
        ...
        if int(self.nwd) <= len(self.tit):
            self.txt = 'same as title'
            self.afz = 0
            return
    ...
    def show(self,tag):
        print('idx:',padsp(self.idx,3),self.tim,self.qid,'nwd:',padsp(self.nwd,5),'flw:',padsp(self.flw,3),self.tit[0:40],'--',self.aut,self.lnk)
        sys.stdout.flush()
    ...
    def process_html_txt(self):
        ...
        # print('TXTXT', gtxt, len(gtxt), txt, len(txt), 'ZZZZ')
        ...
        # elif len(txt) < len(gtxt) - 60:
        elif len(txt) < len(gtxt) - 10:
    ...
    def get_flw_txt(self):
        ...
        dlout(1, 'get_flw_txt for', html_url)
        # if self.TESTING: url = 'file:///home/swang/tmp/content_html.txt_05'
        try:
