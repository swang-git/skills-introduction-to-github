...
class Art:
        ...
        for idx, li in enumerate(lis):
            ...
            if len(flw.txt) > 0:
                ...
                if len(flw.txt) < int(flw.nwd) - 12:
                    dlout(4, 'check_4_MORE_FLW_NWD flw.txt from Art', self.qid, flw.fid, 'len(flw.txt):', len(flw.txt), 'flw.nwd:', flw.nwd, (len(flw.txt) < int(flw.nwd)), 'flw.txt=['+flw.txt+']')
                # print('check_got flw.txt from Ding Art', self.qid, flw.fid, len(flw.txt) < flw.nwd, 'flw.txt=['+flw.txt+']')
                dlout(4, 'dl_GOT flw.txt from Ding Art', self.qid, flw.fid, self.lnk, flw.lnk, 'flw.txt=['+flw.txt+']')
            else:
                dlout(1, 'Empty flw.txt from Ding Art, skip this flw['+flw.txt+']', self.qid, self.fid, self.tit)
        # if self.TESTING:
        # print(len(self.flws))
    ...
    # def get_html_url(self): return 'http://bbs1.people.com.cn/posts/' + self.cat_idx + '/1/2/' + self.fid + '.html'
    def get_txt(self):
    ...
    def show(self, tag): 
        # print(self.item.prettify())
        print('idx:',padsp(self.idx,3),self.tim,self.qid,'nwd:',padsp(self.nwd,5),'flw:',padsp(self.flw,3),self.tit[0:40],'--',self.aut)
        sys.stdout.flush()
    # def show(self, tag):
    #     print(tag, self.idx, self.tim, self.aut, self.lnk, self.nwd, self.dng)
    ...
    def get_flw_txt(self):
        dlout(1, 'get_flw_txt self.nwd', self.nwd)
        hexurl = self.get_html_url()
        dlout(1, 'get_flw_txt for', hexurl)
        # if self.TESTING: url = 'file:///home/swang/tmp/content_html.txt_05'
        try:
            # self.htmltxt = request.urlopen(hexurl)
            req = Request(self.lnk, headers={'User-Agent': 'Mozilla/5.0'})
            self.htmltxt = urlopen(req).read()
            soup = BeautifulSoup(self.htmltxt,  "html5lib")
            dlout(2, 'get_flw_txt.self.txt', soup.get_text())
