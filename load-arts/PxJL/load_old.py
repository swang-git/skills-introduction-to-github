...
for page in range(10, MAX_PAGES):
    ...
    print("article count in page:", len(items))
    # sys.exit(0)
    ...
    for item in items:
        ...
        if art.is_item_for_theday():
            ...
            print(time.strptime(theday, '%Y-%m-%d') > time.strptime(art.tim, '%Y-%m-%d %H:%M'), theday, art.tim)
            print('Processing DONE for', art.qid)
            process_done = True
            break
...
for art in artList:
    i += 1
    print('\n\n>>>>> for', tag, theday + '(' + str(dyx) + ') Processing', str(i) + '/' + str(len(artList)), 'at', get_now(), '=======\n\n')
    art.show('ART')
