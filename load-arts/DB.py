...
def needs_to_get_flws(art):
    rec = session.query(DailyDat.id, DailyDat.flw).filter_by(
        tag=art.tag, ymd=art.ymd, qid=art.qid, flw=art.flw)
    # need to get new flws
    return art.idx == 0 and (rec.scalar() is None or art.flw > rec.flw)


def art_exists(art):
    rec = session.query(DailyDat).filter_by(
        tag=art.tag, ymd=art.ymd, qid=art.qid, flw=art.flw)
    # if rec.scalar() is None: return False  # need to get every thing flws and art.txt
    if rec.scalar() is None or rec.first().txt == '':
        return False  # need to get every thing flws and art.txt
    else:
        return art.flw <= rec.flw        # need to get new flws


def addupdArtItem(art):
    artItem_rec = session.query(ArtItem).filter_by(
        tag=art.tag, pge=art.pge, qid=art.qid)
    if artItem_rec.scalar() is None:
        dlout(3, 'adding ArtItem', art.tag, art.idx,
              art.qid, art.aut, art.nwd, art.tim, art.tit)
        artItem = ArtItem(art)
        session.add(artItem)
        session.commit()
    else:
...
def addDailyDatWX(art):
    ...
    dat_rec = session.query(DailyDat).filter_by(
        tag=art.tag, ymd=art.ymd, qid=art.qid)
    if dat_rec.scalar() is None:
        # dlout(3, 'getting getArt()', art.lnk, art.tit)
        if len(art.txt) < 0 or art.txt is None or art.txt == '':
            print('NO_TXT ADD it anyway', art.idx, art.tag,
                  art.qid, art.tit, art.tim, art.lnk)
            # print('NO_TXT NO_ADDING, return', art.tag, art.qid, art.tit, art.tim, art.lnk)
            # return
        dlout(3, 'adding:DailyDat', art.tag, art.ymd,
        ...
        ####### -- debugging check -- ########
        dat_obj = dat_rec.one()
        arec = session.query(DailyArt).filter_by(
            tag=art.tag, qid=art.qid, fid=art.qid)
        print('TesTing:qid[%d] aut[%s] tit[%s] scalar[%s]' %
              (dat_obj.qid, dat_obj.aut, dat_obj.tit, arec.scalar()))
        if dat_obj.qid > 0:
            print('checkTesTing -- qid:[%d], aut:[%s] tit:[%s]' %
                  (dat_obj.qid, dat_obj.aut, dat_obj.tit))
            # sys.exit(0)
        ######################################
        ...
        if art_rec.scalar() is None:
            ...
            if emptyTXT or autIsWX:
                print('checking DailyArt.txt txt:[%s] qry[%s]' % (
                    emptyTXT, art_rec))
                if emptyTXT:
                    ...
                    session.commit()
                print('upd DailyDat for qid[%s] tit[%s] aut[%s] flw[%s]' % (
                    art.qid, art.tit, art.aut, art.flw))
                datRec = session.query(DailyDat).filter_by(
                    tag=art.tag, ymd=art.ymd, qid=art.qid).first()
                datRec.aut = art.aut
                ...
                print('Exist:DailyArt PXWX, update idx',
    ...
    if art.flws is not None:
        ...
        num_existing_flws = session.query(func.count(DailyArt.idx)).filter_by(
            tag=art.tag, qid=art.qid).scalar() - 1
        if num_existing_flws >= len(art.flws):
            print('there are no new flws, return ...')
            return

        print('ins_upd flws -- existing flws:',
              num_existing_flws, ' < ', len(art.flws))
        art.fsz = 0
        new_flws = []
        for flw in reversed(art.flws):
            print('checking FLW for idx', flw.idx, art.tag,
                  art.qid, flw.fid, flw.tim, flw.aut)
            if art.qid == 8670233 and flw.fid == 1120100:
                continue
            ...
            art_rec = session.query(DailyArt.idx).filter_by(
                tag=art.tag, qid=art.qid, fid=flw.fid)
            # print('CHECK_idx', art_rec.scalar(), flw.idx, art.tag, art.qid, flw.fid, flw.tim, flw.aut)
            print('CHECK_idx', flw.idx, art.tag,
                  art.qid, flw.fid, flw.tim, flw.aut)
            # sys.exit(0)
            try:
                if art_rec.scalar() is None:
                    print('--CK-adding new flw on DailyArt', art.tag, flw.idx, art.qid, flw.fid, flw.aut, flw.tim, flw.txt)
                    new_flws.append(flw)
                # else:  // no need to update the follow ups
                #     try:
                #         print('upding flw on DailyArt', art.tag, art_rec.one(
                #         ).idx, flw.idx, art.qid, flw.fid, flw.aut, flw.tim, flw.txt)
                #     except UnicodeEncodeError:
                #         flw.txt = flw.txt.encode(
                #             'utf-16', 'surrogatepass').decode('utf-16')  # converting emojis
                #         print('\n=X= UnicodeEncodeError upd flw', flw.idx,
                #               art.qid, flw.fid, flw.aut, flw.tim, flw.txt)
                #     art_rec.update({'idx': flw.idx, 'fid': flw.fid})
                #     session.commit()
            except sqlalchemy.orm.exc.MultipleResultsFound:
                print('=X= MultipleResultsFound for ',
                      art.tag, art.qid, flw.fid, 'skip...')
                continue

            art.fsz += int(flw.nwd)
            if flw.img is not None and art.img is not None:
                art.img += flw.img
            if flw.vdo is not None and art.vdo is not None:
                art.vdo += flw.vdo

        art.ffz = art.afz + art.fsz
        art.flw = len(art.flws)
        dat_rec.update({'idx': art.idx, "flw": art.flw, "fsz": art.fsz,
        ...
        for flw in new_flws:
            ...
            try:
                print('adding new flw to DailyArt', art.tag, flw.idx,
                      art.qid, flw.fid, flw.aut, flw.tim, flw.txt)
            except UnicodeEncodeError:
                flw.txt = flw.txt.encode(
                    'utf-16', 'surrogatepass').decode('utf-16')  # converting emojis
                print('\n=X= UnicodeEncodeError add flw', flw.idx,
                      art.qid, flw.fid, flw.aut, flw.tim, flw.txt)
...
def addDailyDatWW(art):
    if len(art.qid) == 3036184_777 or art.txt is None or art.txt == '':
         print('art.tit is bad, skipping, return', art.tag, art.qid, art.tit, art.tim, art.lnk)
         return
    # if len(art.txt) < 0 or art.txt is None or art.txt == '':
    #     print('NO_TXT NO_ADDING, return', art.tag, art.qid, art.tit, art.tim, art.lnk)
    #     return
    dat_rec = session.query(DailyDat.aut, DailyDat.ymd, DailyDat.afz, DailyDat.fsz, DailyDat.ffz,
                            DailyDat.img, DailyDat.vdo).filter_by(tag=art.tag, ymd=art.ymd, qid=art.qid)
    if dat_rec.scalar() is None:
        dlout(3, 'getting getArt()', art.lnk, art.tit)
        art.getArt()
...
def addDailyDatHY(art):
    if len(art.txt) < 0 or art.txt is None or art.txt == '':
        print('NO_TXT NO_ADDING, return', art.tag,
              art.qid, art.tit, art.tim, art.lnk)
        return
    art_rec = session.query(DailyDat.ymd, DailyDat.afz, DailyDat.fsz, DailyDat.ffz,
                            DailyDat.img, DailyDat.vdo).filter_by(tag=art.tag, ymd=art.ymd, qid=art.qid)
    if art_rec.scalar() is None:
        dlout(4, 'adding:DatHY', art.tag, art.ymd,
              art.qid, art.aut, art.afz, art.img, art.tit)
        dat = DailyDat(art)
        session.add(dat)
        session.commit()
...
def addDailyDatQG(art):
    if art == 'testing':
        rec = session.query(DailyDat).filter_by(
            tag='PXZZ', ymd='20171016', qid='2299791').one()
    ...
    art_rec = session.query(DailyDat.afz, DailyDat.fsz, DailyDat.ffz, DailyDat.img,
                            DailyDat.vdo).filter_by(tag=art.tag, ymd=art.ymd, qid=art.qid)
    ...
    if art_rec.scalar() is None:
        ...
        dlout(1, 'adding:DAT', art.tag, art.ymd, art.lnk,
              art.qid, art.aut, art.afz, art.img, art.tit)
        dat = DailyDat(art)
        session.add(dat)
        session.commit()
        ...
        if session.query(DailyArt.idx).filter_by(tag=art.tag, qid=art.qid, fid=art.qid).scalar() is None:
            ...
            daily_art = DailyArt(art)
            daily_art.idx = 0
            dlout(3, 'Exist:DAT no Art, adding art', art.lnk,
                  art.flw, art.qid, art.aut, art.tit)
            addDailyArt(daily_art)
    ...
    if art.flws is not None:
        ...
        # art.ffz = art.afz + art.fsz
        dlout(3, 'UPDATING flw, idx', art.idx,
              "flw", len(art.flws), "fsz", art.fsz)
        all_dat_rec = session.query(DailyDat.afz, DailyDat.fsz, DailyDat.ffz,
                                    DailyDat.img, DailyDat.vdo).filter_by(tag=art.tag, qid=art.qid)
        # all_dat_rec.update({'idx': art.idx, "flw": len(art.flws), "fsz": art.fsz})
        all_dat_rec.idx = art.idx
        all_dat_rec.flw = len(art.flws)
        all_dat_rec.fsz = art.fsz
...
def XXX_upd_art_flw_count(art):
    ...
    # print("fsz: %s" %flw_total_nwd)
    print({'flw': flw_cnt, 'fsz': flw_total_nwd, 'ffz': new_ffz})
    tag = art.tag
    qid = art.qid
    # resulte = session.execute(text("UPDATE DailyDat SET flw=flw_cnt, fsz=flw_total_nwd, ffz=new_ffz WHERE tag=tag AND qid=qid"))
    resulte = session.execute(text("UPDATE DailyDat SET flw=:flw_cnt, fsz=:flw_total_nwd, ffz=:new_ffz WHERE tag=:tag AND qid=:qid"),
                        [{"flw": flw_cnt, "fsz": flw_total_nwd, "ffz": new_ffz, "tag": tag, "qid": qid}])
                    # (flw_cnt, flw_total_nwd, new_ffz, tag, qid))
    # art_rec.update({'flw': flw_cnt, 'fsz': flw_total_nwd, 'ffz': new_ffz})
    session.commit()
def upd_art_flw_count(art):
    # art_rec = session.query(DailyDat.fsz, DailyDat.afz).filter_by(tag=art.tag, qid=art.qid)
    art_rec = session.query(DailyDat).filter_by(tag=art.tag, qid=art.qid)
    flw_all_nwd = art_rec.order_by(desc(DailyDat.fsz))
    flw_total_nwd = flw_all_nwd.first().fsz
    new_ffz = flw_all_nwd.first().afz + flw_total_nwd
    flw_cnt = flw_rec = session.query(func.count(DailyArt.id)).filter_by(
        tag=art.tag, qid=art.qid).scalar() - 1
    # print('updating flw count for', art.ymd, art.tag, art.qid, art.tim, flw_total_nwd)
    # print({'flw': flw_cnt, 'fsz': flw_total_nwd, 'ffz': new_ffz})
    art_rec.update({"flw":flw_cnt, "fsz":flw_total_nwd, "ffz":new_ffz})
...
def get_art_flw_count(art):
    flw_rec = session.query(func.count(DailyArt.id)).filter_by(
        tag=art.tag, qid=art.qid)
    cnt = flw_rec.scalar()
...
def addDailyArtQG(flw, art_rec):
    ...
    if ex == e3:
        print('SKIP_insert for duplicated (idx-qid-fid) for ',
              flw.tag, flw.idx, flw.qid, flw.fid)
        return
    flw_rec = session.query(DailyArt.id, DailyArt.nwd, DailyArt.txt).filter_by(
        tag=flw.tag, qid=flw.qid, fid=flw.fid)
    if flw_rec.scalar() is None:
        ...
        if int(flw.nwd) - 2 > len(flw.txt) or len(flw.txt) == 0:
            ...
            img += flw.img
            vdo += flw.vdo
            dlout(1, 'Get MORE TEXT - get_txt for flw.nwd', flw.qid,
                  flw.fid, 'nwd', int(flw.nwd), 'img:', flw.img, 'vdo:', flw.vdo)
            ...
            print('adding:ArtQG to DB', flw.qid, flw.fid, flw.nwd,
                  flw.get_html_url(), flw.txt, flw.img, flw.vdo)
        if (len(flw.txt) == 0 or flw.txt == '' or flw.txt is None):
            print('got empty flw.txt', flw.qid, flw.fid,
                  'flw.txt=['+flw.txt+']', len(flw.txt), flw.nwd, 'return ...get it next round')
            return
            # sys.exit(-1)
...
def addDailyDat(dat):
    rec = session.query(DailyDat).filter_by(
        tag=dat.tag, ymd=dat.ymd, qid=dat.qid)
    if rec.scalar() is None:
        ...
        rec.update({'idx': dat.idx, "flw": dat.flw, "afz": dat.afz, "fsz": dat.fsz,
                   "ffz": dat.ffz, "img": dat.img, "vdo": dat.vdo})  # ,"":dat.,"":dat.,)
        session.commit()


def addDailyArt(art):
    flw = session.query(DailyArt.id, DailyArt.idx, DailyArt.txt).filter(DailyArt.tag==art.tag, DailyArt.qid==art.qid, DailyArt.fid==art.fid)
    if flw.scalar() is None:
        ...
        if (art.tag == 'PXHY' and art.nwd is not None and int(art.nwd) > 0) or (art.txt is not None and len(art.txt) >= 0):
            ...
            print('NO_adding:Art, NO_TXT', art.tag,
                  art.qid, art.fid, art.nwd, art.tim)
...
def addHomePage(dat):
    ...
    if hop.scalar() is not None:
        ...
        hop.update({"cnt": dat.cnt, 'flw': dat.flw, 'fsz': dat.fsz,
                   'afz': dat.afz, 'ffz': dat.ffz, 'img': dat.img, 'vdo': dat.vdo})
        session.commit()
    else:
...
def getCount(tag, ymd):
    cnt_rec = session.query(func.count(DailyDat.id)
                            ).filter_by(tag=tag, ymd=ymd)
