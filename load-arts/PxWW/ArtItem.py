...
class Art:
    ...
    def __init__(self, idx, lnk, tag, ymd, tit='tit_tesing', tim='2017-10-29', TESTING=False):
        print('lnk=', lnk)
        # if re.match('(.*)/(\d+)\.html', lnk) == None: return
    ...
    def getArt(self):
        ...
        if self.tim > '2022-12-15 17:54': self.bs = divz
        else: self.bs = divz.find('table', attrs={'border':'0','width':'675'})
        ...
        x = self.bs.find('span', class_='STYLE3')
        if x is None: x = self.bs.find('h1', class_='STYLE3')
        tit = x.get_text().strip()
        if tit is not None: self.tit = tit

        # timautline = self.bs.find('span', class_='STYLE4').get_text().strip()
        x = self.bs.find('div', class_='STYLE4')
        if x is None: x = self.bs.find('span', class_='STYLE4')
        timautline = x.get_text().strip()
        # print(timautline)
