...
class Art:
    ...
    def getArt(self):
        ...
        topline = self.bs.find('span', class_='STYLE4')
        timautline = topline.get_text().strip()
        dlout(3, timautline)
        self.tim = re.match('(.*)(\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d)\s+(.*)\s+|(.*)', timautline).group(2)
        dlout(3, 'tim', self.tim)
        x = timautline.split(' ')  ## based on gbk space
        self.aut = x[4]
        dlout(3, 'aut', self.aut)
        # self.flw = re.match('^(\d+).*', x[6]).group(1)
        # flw = topline.find('span', class_="num")
        # dlout(3, 'topline', flw, topline)
        # self.flw = int(flw.text)
        # dlout(3, 'flw', self.flw)
        # if (self.flw > 0):
            # self.process_flw()
            # sys.exit(0)
        self.process_flw()
    ...
    def process_flw(self):
        if float(self.qid) == 2247541: print('[%s][%s]'%(self.qid, self.soup.prettify()))
        dis = self.soup.find_all('div', class_='discuss1')
        # dis = self.soup.find_all('div', { "class": "discuss1" })
        # dis = self.soup.select('div', { "class": "discuss" })
        # dis = self.soup.select('div.discuss1')
        print('call process_flw()[%s][%s]'%(self.qid, dis))
        if self.qid == 2247541:
            print(dis)
            sys.exit(0)
        # flw = topline.find('span', class_="num")
