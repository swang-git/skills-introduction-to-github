...
from urllib import request
from ArtItemQGZJ_test import Art
import time
...
test_site = 'file:///home/swang/Documents/qglt_2019_09_23.html'
...
for page in range(1, MAX_PAGES):
    ...
    for item in items:
        ...
        dlout(2, 'processing item', idx, theday)
        # if art.is_item_for_theday() and art.qid is not None:
        if art.is_item_for_theday():
            dlout(2, 'is_item_4_theday', idx, theday, art.qid, art.lnk)
            art.parse_and_set_attrs()
            art_flw_cnt = DB.get_art_flw_count(art)
            # if TESTING and art.qid == 171806107: # force to get item for testing
            #     art.show('ART')
            #     for flw in art.flws: flw.show('FLW', flw)
                # sys.exit(-1)
            # if TESTING: # force to get item for testing
                # art.dng = True
                # art_flw_cnt = 6
            # if  art.dng and art.dng and int(art.flw) > 0 and art_flw_cnt  < int(art.flw) + 1:
            # if art.dng and int(art.flw) > 0 and art_flw_cnt  < int(art.flw) + 1:
            # if int(art.flw) > 0 and int(art.flw) > art_flw_cnt: 
            if int(art.flw) > 0 and art_flw_cnt < int(art.flw):
                dlout(1, 'art_flw_cnt for ding art if', art.flw, '>', art_flw_cnt, 'needing get MORE for', art.tit, art.qid)
                art.open_and_get_flw()
            elif art.dng and int(art.flw) > 0 and art_flw_cnt == int(art.flw):
                dlout(1, 'updating flw count for', art.tim, art.ymd, art.tag, art.qid, art.tit, art.qid)
...
for art in artList:
    ...
    # if TESTING and art.qid == 171775159 and art.flws is not None and len(art.flws) > 0:
