...
class Art:
    ...
    def __init__(self, idx, lnk, tag, ymd, tit='tit_tesing', tim='2018-11-21', TESTING=False):
        ...
        else: self.qid = int(re.match('(.*)test(\d+)\.html', lnk).group(2))
        self.fid = self.qid
        self.afz = 0
        self.fsz = 0
        self.ffz = 0
    ...
    def process_txt(self):
        ...
        # self.afz = sys.getsizeof(self.txt) - self.img * 120
        # self.afz = 2 * len(self.txt) ## - self.img * 120  # number of bytes
        self.nwd = self.afz
    ...
    def getArt(self):
        ...
        tim = self.soup.find_all('time')[0].get_text()
        self.tim = tim.strip()
