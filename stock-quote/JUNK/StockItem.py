...
class Stock:
    ...
    def getQuote(self, symbol):
        ...
        # price = soup.find('fin-streamer', {'data-symbol': 'MSFT'}).text
        # print('price = %s'%price)
        # print(soup.find_all('fin-streamer', {'data-symbol': 'MSFT'}))
        price=soup.find('fin-streamer', {'data-symbol': 'MSFT', 'data-field':'regularMarketOpen'}).text
        print('price = %s'%price)
        prchange=soup.find('fin-streamer', {'data-field':'regularMarketChange'}).text
        print('prchange = %s'%prchange)
        prchangepct=soup.find('fin-streamer', {'data-field':'regularMarketChangePercent'}).text
        print('prchangepct = %s'%prchangepct)
        dayRange=soup.find('fin-streamer', {'data-symbol': 'MSFT', 'data-field':'regularMarketDayRange'}).text
        print('dayRange = %s'%dayRange)
        range52WK=soup.find('fin-streamer', {'data-symbol': 'MSFT', 'data-field':'fiftyTwoWeekRange'}).text
