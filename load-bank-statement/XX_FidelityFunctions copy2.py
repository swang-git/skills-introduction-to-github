...
def checkAccountEndBalance(self, acIdx, hcnt):
  dg('-dg- checkAccountEndBalance():')
  abal = 0.0
  ebal = 0.0
  for i, h in enumerate(self.holdings[hcnt:]):
    dg('-dg-holdings: ac%d, %s %s'%(acIdx, padsp(str(i), 2), h))
    # for h in self.holdings:
    if h[4]: abal += h[4]
    ebal += h[5]
  ast = self.accountAssets[acIdx]
  try: assert round(ebal - ast.ebal, 2) == 0
  except:
    print('\n\t checkAccountEndBalance() FAILED:')
    print('\t ast.ebal == ebal')
    print('\t end balance check: round(%.2f - %.2f, 2)'%(ast.ebal, ebal))
