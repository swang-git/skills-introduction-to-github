-- CALL Golf_dev.get_tournament_list(NULL)
-- CALL Golf.get_players_ranking(49);
use Golf_dev;
drop procedure IF exists get_tournament_list;

DELIMITER $$
CREATE DEFINER=`swang`@`localhost` PROCEDURE `get_tournament_list`(tid INT)
