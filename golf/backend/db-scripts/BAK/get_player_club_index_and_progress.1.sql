USE princeton_golf;
DROP PROCEDURE IF EXISTS get_player_club_index_and_progress;

DELIMITER $$

USE princeton_golf;
CREATE DEFINER=`swang`@`%` PROCEDURE get_player_club_index_and_progress(tid INT, pid INT)
CREATE DEFINER=`pccadmin`@`%` PROCEDURE `get_player_club_index_and_progress`(tid INT, pid INT)
BEGIN
...
END IF;
SELECT club_index, year_progress;
...
CALL get_player_club_index_and_progress(68, 9);
