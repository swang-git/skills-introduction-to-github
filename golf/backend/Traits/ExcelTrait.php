...
trait ExcelTrait {
  private $pcount = 0;
  private $latestGameDateOnSheet = null;
  public function processKjExcelFile($xlsxfile) { Log::info("processKjExcelFile for $xlsxfile");
    ...
    $this->latestGameDateOnSheet = $latestGameDateOnSheet;
    ...
    $range = $sheet->rangeToArray("A1:H$lastDataRow"); // note this G
  ...
  private function getGameDataForDate($playerData, $range, $nextGameDate) {
    ...
    foreach ($range as $row) {
      ...
      $dataRow = [
        ...
        'avg_score' => $pda[6],
        'net_team_win_loss' => $pda[7],
        'net_pair_win_loss' => $pda[8],
  ...
  private function getMpIdAndCheckNewPlayer($name) {
    list($lastn, $firstn) = explode(', ', $name);
      ...
      // $xp = Player::where([['players.lastname', $lastn], ['firstname', $firstn], ['players.status','A'], ['match_players.status', 'A']])
      ...
      // $xk = KJNewPlayer::where([['lastname', $lastn], ['firstname', $firstn], ['kj_new_players.status','A'], ['match_players.status', 'A']])
      // ->leftJoin('match_players', function($leftJoin) {
      //   $leftJoin->on('match_player_id', 'match_players.id');
      $x = MatchPlayer::where([['lastname', $lastn], ['firstname', $firstn], ['match_players.status', 'A']])->select('id')->get();
			if (count($x) != 1) {
				// $aliases = MatchPlayer::where('status', 'A')->select('alias')->get();
				Log::info("$lastn", [ 'firstname' => $firstn, 'lastname' => $lastn, 'status' => "ADD_NEW_SIM_PLAYER"]);
				return [ 'firstname' => $firstn, 'lastname' => $lastn, 'status' => "ADD_NEW_SIM_PLAYER"];
			}
      // $xx = $x->toArray()[0];
			// $fullname = $lastn .', '.$firstn;
			// $alias = $xx['alias'];
			// $match_player_id = DB::select('select id from match_players_view where name = "$fullname" and match_id = 14');
			// $match_player_id = Player::where([['players.lastname', $lastn], ['players.firstname', $firstn], ['players.status', 'A'], ['match_players.status', 'A']])
			$mpId = Player::where([['players.lastname', $lastn], ['players.firstname', $firstn], ['players.status', 'A'], ['match_players.status', 'A']])
			  ->leftJoin('match_players', function($leftJoin) {
          $leftJoin->on('match_players.player_id','players.id');
				})->select('match_players.id')->value('id');
      if (is_null($mpId)) $mpId = $x = MatchPlayer::where([['lastname', $lastn], ['firstname', $firstn], ['status', 'A']])->value('id');
      return ['match_player_id' => $mpId, 'status' => 'mpId'];
  }
  private function getPlayerData($range) { //Log::debug("getPlayerData");
    ...
    foreach ($range as $row) {
      ...
      if ($retval['status'] == 'ADD_NEW_SIM_PLAYER') {
        $firstn = $retval['firstname'];
        $lastn = $retval['lastname'];
        // $aliases = $retval['aliases'];
        ...
        // $lastMatchPlayerId = MatchPlayer::where('status', 'A')->orderBy('id', 'DESC')->value('id');
        $mpdm = new MatchPlayer;
        $mpdm->match_id = 14;
        $mpdm->firstname = $firstn;
        $mpdm->lastname = $lastn;
        // $mpdm->player_id = 9999999;
        // $mpdm->alias = 'XYZ';
        $mpdm->save();
        $mpId = $mpdm->id;
        // Log::debug("need to add new player for firstname=$firstn lastname=$lastn lastMpId=$lastMatchPlayerId status='ADD_NEW_SIM_PLAYER'", [__line__]);
        Log::debug("add MatchPlayer and will update from frontend for firstname=$firstn lastname=$lastn mpId=$mpId status='ADD_NEW_SIM_PLAYER'", [__line__]);
        Log::debug("add KjNewPlayer and update from frontend for firstname=$firstn lastname=$lastn mpId=$mpId status='ADD_NEW_SIM_PLAYER'", [__line__]);
        $kndm = new KjNewPlayer;
        $kndm->match_player_id = $mpId;
        $kndm->firstname = $firstn;
        $kndm->lastname = $lastn;
        $kndm->game_id = 14;
        $kndm->game_date = $this->latestGameDateOnSheet;
        $kndm->save();
        // exit(0);
      ...
      $num_game_played = $row[1] == '' ? null : $row[1];
      $net_team_win_loss = $row[2] == '' ? null : $row[2];
      $net_pair_win_loss = $row[3] == '' ? null : $row[3];
      $weighted_avg_idx = $row[4] == '' ? null : $row[4];
      $all_game_played = $row[5] == '' ? null : $row[5];
      $avg_idx_all_games = $row[6] == '' ? null : $row[6];
      $avg_score_all_games = $row[7] == '' ? null : $row[7];
      // Log::info("$name, mp_id=$mpId, $num_game_played, $weighted_avg_idx, $all_game_played, $avg_idx_all_games, $avg_score_all_games");
      // exit(0);
      $rdata = [$name, $mpId, $num_game_played, $weighted_avg_idx, $all_game_played, $avg_idx_all_games, $avg_score_all_games, $net_team_win_loss, $net_pair_win_loss];
