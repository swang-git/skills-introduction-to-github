    ...
    <q-header elevated class="bg-cyan-8">
      <q-toolbar>
        <q-btn
          ...
          icon="menu"
        />
        <q-btn v-else to="/" round dense glossy color="blue-6"
          ><q-icon name="🏠" style="margin: -12px 0 0 0"
        /></q-btn>
        <q-toolbar-title class="cursor-pointer" @click="reloadPage()">{{
          pageTitle
        }}</q-toolbar-title>
        <q-input
          v-if="showSearch"
          :style="isIM ? { width: '90px' } : { width: '200px' }"
          dark
          borderless
          v-model="searchQuery"
        ...
        <q-btn
          v-if="pageTitle == 'Course Details' && SysAdmin"
        ...
        <q-btn
          v-if="SysAdmin && updateUserGuide"
          outline
          rounded
          dense
          icon-right="update"
          :label="isDesk ? 'update user guide' : 'user guide'"
          @click="updateUserGuide()"
          class="q-pl-sm"
        />
      </q-toolbar>
    </q-header>
    ...
    >
      ...
      >
        <q-list padding>
          ...
          <q-item
            clickable
            v-ripple
            @click="openApp('PlayerList', 'Player List')"
          >
            <q-item-section avatar>
              <q-icon name="person" color="orange-9" size="30px" />
            </q-item-section>
            <q-item-section> Player List </q-item-section>
          </q-item>
          ...
          <q-item
            clickable
            v-ripple
            @click="openApp('TournamentList', 'Tournament List')"
          >
            <q-item-section avatar>
          ...
          <q-item
            clickable
            v-ripple
            @click="openApp('CourseDetails', 'Course Details')"
          >
            <q-item-section avatar>
              <q-icon name="golf_course" color="green-9" size="30px" />
            </q-item-section>
            <q-item-section> Golf Courses </q-item-section>
          </q-item>

          <q-item
            clickable
            v-ripple
            @click="openApp('Signup', 'Sign up Games')"
          >
            <q-item-section avatar>
              <q-icon name="assignment" color="pink" size="30px" />
            </q-item-section>
            <q-item-section> Signup Game </q-item-section>
          </q-item>
          ...
          <q-item
            clickable
            v-ripple
            @click="openApp('JZsMatch', 'JZ\'s Match', 13)"
          >
            <q-item-section avatar>
              <q-icon name="img:icons/JZ.png" size="30px" />
            </q-item-section>
            <q-item-section> JZ's Match </q-item-section>
          </q-item>

          <q-item
            clickable
            v-ripple
            @click="openApp('KJsMatch', 'KJ\'s Match', 14)"
          >
            <q-item-section avatar>
              <q-icon name="img:icons/KJ.png" size="30px" />
            </q-item-section>
            <q-item-section> KJ's Match </q-item-section>
          </q-item>

          <q-item
            clickable
            v-ripple
            @click="openApp('ALsMatch', 'AL\'s Match', 16)"
          >
            <q-item-section avatar>
              <q-icon name="img:icons/AL.png" size="30px" />
            </q-item-section>
            <q-item-section> AL's Match </q-item-section>
          </q-item>

          <q-item
            clickable
            v-ripple
            @click="openApp('PGCGroupList', 'PGC Group List')"
          >
            <q-item-section avatar>
              <q-icon name="img:icons/PGC.png" color="cyan-9" size="30px" />
            </q-item-section>
            <q-item-section v-if="PGCsAdmin"> Do Grouping </q-item-section>
          ...
          <q-item
            ...
            v-ripple
            @click="
              openApp('PGCGameList', 'Club Outings, Tournaments and PlayOff')
            "
          >
            <q-item-section avatar>
          ...
          <q-item
            clickable
            v-ripple
            @click="openApp('PGCGameRules', 'PGC Game Rules')"
          >
            <q-item-section avatar>
          ...
          <q-item
            v-else-if="
              !(SysAdmin || JZsAdmin || KJsAdmin || ALsAdmin || PGCsAdmin)
            "
            clickable
            v-ripple
          ...
          <q-item
            ...
            clickable
            v-ripple
            @click="openApp('LoadLogPage', 'Show Log Page', 'Golf Logs')"
          >
            <q-item-section avatar>
          ...
          <q-item
            v-if="compVer == undefined"
            clickable
            v-ripple
            @click="openApp('DevTest', 'Testing New', 'Testing New')"
          >
            <q-item-section avatar>
      ...
      >
        <div class="absolute-bottom bg-transparent">
          ...
          /></q-avatar>
          <q-avatar size="38px" class="q-mb-sm" style="padding: 0 0 0 105px"
...
import holidays from '../../src/components/HolidayDialog.vue'
import InfoDisplay from '../../src/components/InfoDisplay.vue'
import LoginDialog from '../../src/pages/LoginDialog.vue'
import UserGuidePad from '../../src/components/UserGuidePad.vue'
import RegisterDialog from '../../src/pages/RegisterDialog.vue'
...
emitter.on('cur-tit', ctit => {
  setTitle(ctit)
  setInterval(setTitle, oneHour, ctit)
})
...
function getUserType() {
  const path = ENV_API + '/golf/getUserType'
  gaxios(path)
}
function logout() {
...
function openApp(app, pageTitle, gameId = null) {
  ...
  if (app == 'DevTest') {
    ...
    const path = ENV_API + '/DevTest/processKjExcel/' + xlsxFile
    return gaxios(path)
  }
  // gameId.value = gameId
  if (appname.value === 'PGCGameRules') {
    return showPGCRules()
  }
...
//   const path = ENV_API + '/golf/getUserGuideId/' + pagename.value
//   gaxios(path)
// }
