<template>
  <div class="bg-teal-10">
    ...
    <q-input
      ...
      v-model="course.name"
      dark
      label="Golf Coruse Name"
      class="text-h6 q-px-sm q-mx-sm"
    >
      <div class="q-pl-md">
        <q-btn
          v-if="createNewCourse"
          glossy
          no-wrap
          label="Create Course"
          @click="addCourseTemplate()"
          class="q-mt-sm q-ma-sm text-cyan-2 bg-red"
        ></q-btn>
        <q-btn
          v-else
          rounded
          ...
          no-wrap
          label="Select Course"
          @click="showCourseOptions"
          class="q-ma-sm text-white bg-cyan-9"
        ></q-btn>
      </div>
    </q-input>
  </div>
  <div v-if="!createNewCourse">
    <div v-if="isDesk" class="q-pa-xs bg-teal-9">
      <div class="bg-teal-9" dense style="margin: -2px 4px 2px 4px">
        ...
        <div
          v-for="(t, i) in course.trys"
          :key="t.id"
          class="row"
          :class="t.teebox === teebox ? getBFColor() : 'bg-teal'"
        >
          <div class="text-center" style="width: 20%">
            <q-btn size="18px" no-caps flat dense @click="setTeebox(i)" :disable="!SysAdmin">{{
              t.teebox
            }}</q-btn>
          </div>
          <div class="text-center" style="width: 20%">
        ...
        <q-card-actions v-if="SysAdmin" align="between">
          ...
          <q-btn @click="addTee" glossy color="blue-9" no-caps icon-right="text_fields"
            >Add Teebox</q-btn
          >
        </q-card-actions>
      </div>
      <table class="q-pa-xs" style="margin: 0 auto">
        <q-tr>
          <q-th colspan="9">
            <q-btn
              ...
              v-if="courseSelected"
              :class="getBFColor()"
              @click="getCourseYardage()"
            >
              Yardage for {{ teebox }} Tee:
              <span v-if="fyard > 0">&nbsp;{{ fyard }} + {{ byard }} = {{ fyard + byard }} </span
              ><span v-else>~ No Yardage yet</span>
        ...
        <q-tr v-if="courseSelected">
          <q-td v-for="i in f9" :key="i">
            <q-btn
              ...
              :label="yard(i)"
              :class="i == setYardi || setYardi > 0 ? yardColor[i] : getBFColor()"
              outline
              @click="setYard(i)"
            >
              <Tooltip :txt="Hole(i)" />
        ...
        <q-tr
          ><q-th colspan="9" class="th-title text-h6 text-white text-center"
            >Pars of Hole 1 to 18 <span v-if="fpars > 0">({{ fpars }} + {{ bpars }})</span></q-th
          ></q-tr
        >
        <q-tr v-if="courseSelected">
          <q-td v-for="i in f9" :key="i.x">
            <q-btn
              ...
              color="cyan-2"
              outline
              :class="selectedParsColor[i]"
              @click="setPar(i)"
            >
              <q-chip
                class="text-black text-body2"
                style="width: 25px; height: 25px; padding-left: 9px"
                color="grey"
              >
                {{ i }}
              </q-chip>
      ...
      <q-btn-group v-if="SysAdmin && courseSelected" glossy spread class="q-px-xs q-gutter-md">
        <q-btn
          ...
          color="red-9"
          rounded
          :disable="disableDelButton"
        />
        <q-btn
          align="center"
          label="update"
          @click="checkInput('upd')"
          icon="update"
          color="green-9"
          :disable="disableUpdButton"
        />
        <!-- <q-btn align="right"  label="create" @click="checkInput('add')" icon-right="create" color="primary" rounded :disable=disableAddButton /> -->
      </q-btn-group>
      <a
        v-if="course.name == 'Bedminster NEW, Trump National GC'"
        href="/docs/golf/scorecard/Bedminster_new_scorecard.png"
        target="_blank"
        >Scorecard</a
      >
      <a
        v-else-if="course.name == 'Bedminster OLD, Trump National GC'"
        href="/docs/golf/scorecard/Bedminster_old_scorecard.png"
        target="_blank"
        >Scorecard</a
      >
      <a
        v-else-if="course.website != null"
        :href="course.website"
        target="_blank"
        class="text-h6 text-cyan q-pl-md"
        >Course Website</a
      >
      <div v-if="course.address != null" class="text-h6 text-white q-pl-md">
        Address: {{ course.address }}
      </div>
      <div v-if="course.phone != null" class="text-h6 text-yellow q-pl-md">
        Phone: {{ course.phone }}
      </div>
    </div>
    <div v-else class="q-pa-xs bg-teal-10">
      <div class="bg-teal-9" style="margin: -2px 4px 2px 4px">
        ...
        >
          <div class="col-3 text-center">
            <q-btn size="19px" no-caps flat dense @click="setTeebox(i)" :disable="!SysAdmin">{{
              t.teebox
            }}</q-btn>
          </div>
          <div class="col-3 text-center">
      ...
      <table class="q-pa-xs" style="margin: 0 auto">
        ...
        <q-tr
          ><q-th colspan="6" class="th-title text-h6 text-white text-center">Handicaps </q-th></q-tr
        >
        <q-tr v-if="courseSelected">
          <q-td v-for="i in f9.slice(0, 6)" :key="i">
            <q-btn
...
function setParsBGColor(i) {
  // console.log(`-CK-fn-setParsBGColor i=${i}, holei=${store.holes[i]}`)
  console.log(`-CK-fn-setParsBGColor i=${i}`, hole(i), course.value.holes) // magic: store.holes changes -> course.value.holes
  // no magic at all: store.holes and course.value.holes point to the same object
  // course.value.holes.h1 = 777
  // console.log(`-CK-fn-setParsBGColor i=${i}`, hole(i), store.holes) // magic: store.holes changes <-! course.value.holes
  ...
  // }
  // console.log(`-CK-setParsBGColor i=${i} totalPars=${totalPars} coursePars=${course.value.trys[0].par}`, store.holes)
}
...
function setCourseDetails(da) {
  ...
  // store.holes = JSON.parse(JSON.stringify(course.value.holes)) // not necessary ?
  store.holes = course.value.holes
  ...
  // store.holes = {h1:777}
  // console.log(`-CK-fn-setCourseDetails course.holes`, store.holes, course.value.holes) // no change for course.value.holes
  fyard.value = 0
  byard.value = 0
  fpars.value = 0
  bpars.value = 0
...
function getBFColor(i = -1) {
  ...
  if (teeColor === 'gold') {
    retval = 'yellow-8'
    txtclr = 'grey-9'
  } else if (teeColor === 'black') {
    retval = 'grey-7'
    txtclr = 'lime-1'
  } else if (teeColor === 'forward') {
    retval = 'red'
    txtclr = 'yellow-2'
  } else if (teeColor === 'scarlet') {
    retval = 'purple-4'
    txtclr = 'white'
  } else if (teeColor === 'blended') {
    retval = 'white'
    txtclr = 'blue-8'
  } else if (teeColor === 'player') {
    retval = 'blue'
    txtclr = 'yellow-2'
  } else if (teeColor === 'silver') {
    retval = 'grey'
    txtclr = 'cyan-2'
  } else if (teeColor === 'blue') {
    retval = 'blue-8'
    txtclr = 'lime-1'
  } else if (teeColor === 'white') {
    retval = 'white'
    txtclr = 'black'
  } else if (teeColor === 'red') {
    retval = 'red'
    txtclr = 'yellow-2'
  } else if (teeColor === 'green') {
    retval = 'green'
    txtclr = 'black'
...
function getCourseYardage() {
  ...
  const path =
    process.env.API + '/golf/CourseYardage/' + courseId + '/' + teeboxId + '/' + teebox.value
  gaxios(path)
}
function checkYardage() {
  // console.log(`-CK-fn-checkYardage teeIdx=${teeIdx} teebox=${teebox.value}`)
  const fbytotal = fyard.value + byard.value,
    ydg = course.value.trys[teeIdx].yardage
  if (fbytotal > 0 && fbytotal !== ydg) {
    console.log(
      `-CK-fn-checkYardage totalYardage=${fbytotal} courseYardage=${ydg} teeIdx=${teeIdx} teebox=${teebox.value}`,
    )
    $q.dialog({
      title: 'Please check yardage: ' + fbytotal + ' != ' + ydg + ' for ' + teebox.value + ' Tee',
    })
    return false
  }
  return true
...
function setPar(i) {
  ...
  console.log(`-CK-fn-setPar i=${i}`, course.value.holes)
...
function setYardBGColor(holeIdx, hyard) {
  console.log(
    `-CK-fn-setYardBGColor fyard=${fyard.value} holeIdx=${holeIdx} hyard=${hyard} course=${course.value.name}`,
  )
  ...
  calcFByards()
  console.log(
    `-CK-fn-setYardBGColor fyard=${fyard.value} byard=${byard.value} holeIdx=${holeIdx} hyard=${hyard}`,
    yard(9),
  )
}
function addTee() {
...
function delTee() {
  // delete the last row in the teeboxes, need to click update button
  ...
  $q.dialog({
    ...
    message:
      '<center style="font-size:20px">Delete <b><em>' +
      del.teebox +
      '</em></b> teebox from <b><em><p>' +
      course.value.name +
      '?</em></b></center>',
    html: true,
    ok: 'OK',
...
function checkInput(act) {
  console.info(
    `-ck-fn-checkInput act=${act} hole1=${hole(1)} h2=${hole(2)} h3=${hole(3)} h4=${hole(4)}`,
  )
  ...
  console.log(`-CK-pars`, pars, store.holes)
  if (totalPar != par) {
    const tit = 'Par is not equal to course par ' + par
    const msg = 'Please check Hole Par settings and Course Par setting ' + totalPar
    emitter.emit('open-InfoDisplay', tit, msg)
  }
  // course.value.trys[teeIdx].holes = pars
  // course.value.holes = pars.map(i => {'h' + i + 1})
  // course.value.holes = store.holes
