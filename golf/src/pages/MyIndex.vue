<template>
  <q-page
    ...
    style="background-image: url('https://cdn.quasar.dev/img/material.png')"
  >
    ...
    >
      ...
      >
        <div class="absolute-bottom bg-transparent">
          <q-avatar size="58px" class="q-pt-xl" style="margin-left: 15px"
            ><img src="icons/boy.png"
          /></q-avatar>
          <span class="text-red text-h6 no-wrap" style="margin: 0px 0 0 -48px">{{ mcount }}名</span>
          <q-avatar size="58px" style="margin: -10px 0 0 266px"
    ...
    <q-card-actions class="row" style="margin: auto; margin-top: -370px" align="between">
      ...
      <RoundButton
        ...
        colr="teal-10"
        ttip="All Club Games"
        @click="openApp('TournamentList')"
      />
      <RoundButton
        size="20px"
        icon="img:icons/hole_in_one.png"
        clas="q-ma-xs q-pb-sm"
        colr="green-9"
        ttip="Golf Course Details"
        @click="openApp('CourseDetails')"
      />
      <RoundButton
        size="20px"
        icon="SG"
        colr="pink"
        clas="q-ma-xs q-pb-sm"
        ttip="Game Registration"
        @click="openApp('Signup')"
      />
      <RoundButton
        size="20px"
        icon="JZ"
      ...
      <RoundButton
        ...
        colr="green-10"
        clas="q-ma-xs q-pb-sm"
        ttip="(Admin)PGC Game Grouping, Entering Scores, etc."
        @click="openApp('PGCGroupList')"
      />
      <RoundButton
        size="20px"
        icon="PG"
        colr="green-10"
        clas="q-ma-xs q-pb-sm"
        ttip="PGC Game List of Expired Games with scores(regular users)"
        @click="openApp('PGCGameList')"
      />
      <RoundButton
        size="20px"
        icon="PR"
      ...
      <RoundButton
        ...
        ttip="Admin Login"
        @click="login()"
        v-if="!(JZsAdmin || SysAdmin || PGCsAdmin)"
      />
      <RoundButton
        size="20px"
        icon="logout"
      ...
      <RoundButton
        ...
        colr="blue-10"
        clas="q-ma-xs"
        ttip="App Details for Each Button"
        @click="opened = !opened"
      />
      <q-btn
        size="20px"
        ...
        <q-list class="bg-cyan-1">
          ...
          <q-item>
            ...
            <q-item-section class="cursor-pointer" @click="openApp('ALsMatch')">
              ...
              <q-item-label caption
                >Alan Long team matches/groups/team score personal avg/score</q-item-label
              >
            </q-item-section>
          </q-item>
          <q-item>
          ...
          <q-item>
            ...
            <q-item-section class="cursor-pointer" @click="showPGCRules()">
              ...
              <q-item-label caption
                >The rules applied to all PGC games unless otherwise specified</q-item-label
              >
            </q-item-section>
          </q-item>
          <q-item>
...
const { store, isIM, SysAdmin, PGCsAdmin, JZsAdmin } = libFunctions()
...
console.log(
  `-ST-MyIndex JZsAdmin=${JZsAdmin.value} SysAdmin=${SysAdmin.value} PGCsAdmin=${PGCsAdmin.value}`,
)
...
emitter.on('golf-getPlayerCount', (x) => {
  mcount.value = x.mcnt
  fcount.value = x.fcnt
})
