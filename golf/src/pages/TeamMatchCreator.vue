...
var tmnt = reactive({ start_at: teeTime, fees: 22, course_id: 0, teetime_gaps: [0] })
...
function add() {
  ...
  const msg =
    '<div class="text-center">Start at ' +
    tmnt.start_at +
    '</div><div class="text-center">on Course: ' +
    tmnt.courseName +
    '</div>'
...
function setTmntFees() {
  ...
  console.log(
    `%c-CK-setTmntFees() teeTime=${teeTime.value} course=${course} test=${/Mercer Oak/.test(course)}`,
    'color:lime;font-size:medium',
  )
  if (/Mercer Oak|Hopewell|Mountain View/.test(course))
    tmnt.fees = teeTime.value.isWeekend() ? 32 : 22
...
emitter.on('golf-CourseList', (x) => {
  courseList.value = x.lst
})
emitter.on('golf-TeeboxList', (x) => {
  teeboxList.value = x.lst
})
