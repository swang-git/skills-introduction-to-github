USE princeton_golf;
DROP PROCEDURE IF EXISTS create_tmp_table_club_index;

DELIMITER $$

USE princeton_golf;
CREATE DEFINER=`pccadmin`@`%` PROCEDURE `create_tmp_table_club_index`(tid INT, year_begin BOOLEAN)
BEGIN
...
DEClARE pid_cursor CURSOR FOR 
	SELECT p.id as pid FROM -- , CONCAT(p.lastname, ', ', p.firstname) as player FROM 
        tplayers tp
...
ELSE
    ...
    -- CREATE TEMPORARY TABLE tmp_club_index_yb(pid INT, player VARCHAR(32), num_games INT, club_index DECIMAL(3,1), calc_start DATETIME, tmnt_start DATETIME);
...
CALL create_tmp_table_club_index(68, 0); -- regular club index
SELECT * from tmp_club_index;

CALL create_tmp_table_club_index(68, 1); -- year begin club index
SELECT * from tmp_club_index_yb;
