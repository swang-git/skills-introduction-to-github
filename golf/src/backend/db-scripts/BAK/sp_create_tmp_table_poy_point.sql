USE princeton_golf;
DROP procedure IF EXISTS create_tmp_table_poy_point;

DELIMITER $$
USE princeton_golf$$

CREATE DEFINER=`pccadmin`@`%` PROCEDURE create_tmp_table_poy_point(tid INT, pid INT)
BEGIN
...
CALL create_tmp_table_poy_point(68, -1);
SELECT * FROM tmp_poy_point;
