...
class GolfController extends Controller {
	...
	public function getScoreId(Request $da) { Log::info('getScoreId', [$da->playerId, $da->courseId, $da->teetime]);
		...
		$courseId = $da->courseId;
		$teetime = $da->teetime;
		$dx = Score::where([ ['player_id', $playerId], ['teetime', $teetime], ['course_id', $courseId] ])->select('id as scoreId')->get(); 
		Log::info($dx[0]['scoreId']);
		if (count($dx) == 1) {
  ...
  protected function validator(array $data) {
	...
	private function updTPlayerScore($da, $gross_score, $hdcpdiff, $scoreId=0, $datpid=0) {
		Log::info("updTPlayerScore, tpid=$datpid, scoreId=$scoreId, gross_score=$gross_score, hdcpdiff=$hdcpdiff", $da->toArray());
		$tpid = $da['id'];
