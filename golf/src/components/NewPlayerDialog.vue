  ...
  <q-dialog v-model="opened" seamless :maximized="isIM">
    <div class="bg-teal-9 q-pa-sm" style="width: 400px">
      <q-toolbar glossy>
        ...
        <q-toolbar-title v-if="isClubMember" class="text-white text-h6">
          {{ year }} Membership ({{ action }})
        </q-toolbar-title>
        <q-toolbar-title v-else class="text-white text-h6 text-center"
          >Add New Player for Team Match</q-toolbar-title
        >
      </q-toolbar>
        ...
        <q-card-section style="margin-top: -24px">
          <div class="row">
            ...
            <q-input
              outlined
              rounded
              :class="{ 'col-6': isClubMember, 'col-12': !isClubMember }"
              v-model="phone"
              dark
              label="Phone"
              type="tel"
...
function openIt(act, m, isClubMem = true, gId) {
  console.log(
    `-fn-openIt act=${act} isClubMember=${isClubMem} gameId=${gId}`,
    m
  )
  action.value = act
...
function checkInput() {
  ...
  if (m.chname !== null && m.chname != undefined && m.chname.length > 8) {
    ...
    tit =
      'lastname is required and cannot be more than 16 letter and at least 1 letters'
    msg = 'please revise your input.'
    isMemberInfoOK = false
  } else if (m.firstname.length < 1 || m.firstname.length > 16) {
    tit =
      'firstname are required and cannot be more than 16 letter and at least 1 letters'
    msg = 'please revise your input.'
    isMemberInfoOK = false
