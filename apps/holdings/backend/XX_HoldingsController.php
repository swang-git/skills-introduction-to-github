...
use App\Models\holdings\FidelityAccountActivity;
...
class HoldingsController extends Controller {
	...
	public function getTransDetails($bank, $year, $month) { Log::info('getTransDetails', [$bank, $year, $month, Auth::user()->id]);
		$userId = Auth::user()->id;
		$dats = FidelityAccountActivity::where([ ['user_id', $userId], ['bank', $bank], ['year', $year], ['month', $month], ['status', 'A'] ])
			->select('bank', 'year', 'month', 'account_name', 'account_num', 'description', 'sett_date', 'security', 'amount')->orderBy('sett_date')
			->orderBy('account_num')
			->get();
		// Log::info('trans details', $dats->toArray());
		return ['dats' => $dats, 'bank' => $bank, 'year' => $year, 'month' => 'month'];
	}
	public function getList() {
		...
		$holds = BankStatementHolding::select('symbol', 'end_balance')
				...
				->get();
		foreach($holds as $h) {
			if (in_array($h->symbol, ['SPAXX', 'SPRXX', 'FZDXX', 'FDRXX', 'QBNYQ', 'FNJXX'])) {
				$mmaket += $h->end_balance;
			}
		}

		return ['stocks' => $stocks, 'mmaket' => $mmaket, 'dats' => Collect($dats)];
	}
	public function getDetails($bank, $year, $month) { Log::info('Holdings - getDetails', [$bank, $year, $month]);
		$userId = Auth::user()->id;
		...
		return ['notes' => $notes, 'chkactvs' => $chkactvs, 'savactvs' => Collect($savactvs) ];
	}

	public function XXXgetHoldings($bank, $year, $month) { Log::info('Holdings - getHoldings', [$bank, $year, $month]);
		$userId = Auth::user()->id;
		if ($bank == 'Fidelity') {
			...
			$holdings = FidelityAccountActivity::where([['status', 'A'], ['user_id', $userId], ['bank', $bank], ['year', $year], ['month', $month]])
					->select('symbol', 'account_name', 'account_num', 'quantity', 'price', 'start_balance', 'end_balance')
					->selectSub($diffSql, 'diff')
					->selectSub($costSql, 'cost')
					->orderBy('account_num')->get();
		}
		return ['holdings' => Collect($holdings) ];
	}
