...
<q-dialog v-model="opened" transition-show="slide-right" persistent>
  <q-layout container class="bg-teal-10 fixed-center" :style="{ height:isDesk ? '300px' : '600px', width:isDesk ? '488px' : '' }">
    ...
    <q-page-container v-if="isDesk" class="">
      <q-page>
        <div class="row">
          <DatePicker class="col-7 q-pl-xs q-pt-xs" label="TODO Date" :date="row.date" @upd-date="updDate" txsz="text-h6" />
...
console.log('-ST-htdar')
emitter.on('open-htdar', (act, row) => openIt(act, row))

const emit = defineEmits ([
  'added-row', 'upded-row', 'del-confirmed', 'upd-link', 'save-details'
])

emitter.on('ht-add', (da) => emit('added-row', da.row))
emitter.on('ht-upd', (da) => emit('upded-row', da.row))
emitter.on('ht-del', (da) => emit('del-confirmed', da.row))
emitter.on('ht-del-row', (rw) => { row.value = rw; del() })

function openIt (act, rw) {
