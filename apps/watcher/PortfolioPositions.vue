  ...
  >
    ...
    <template v-slot:top-right>
      ...
      <table style="margin-left:82px">
        ...
        <q-tr>
          <td>总获：</td><td class="text-h6 text-right">{{ totalGL }}</td>
          <td class="text-right" style="width:100px">误差：</td>
          <td>
            <span @click="updPortfValue" class="cursor-pointer bg-indigo-9 q-py-xs q-px-sm">{{ ppdiff }}
              <q-tooltip class="bg-indigo-9 text-h6">Click to Update Portfolio Value / Weight</q-tooltip>
          </span>
          </td>
        </q-tr>
        ...
        <q-td class="bg-teal-10" colspan="100%">
          <table style="width:99.7%;border:2px solid cyan">
            <q-tr>
              <td colspan="4">
                <q-card-actions align="between">
                  <div style="width:96%">
                    <div class="text-center">Account: {{ p.row.account_name }} ~ {{ p.row.account }} ~ {{ p.row.symbol }} ~ {{ p.row.date }}</div>
            ...
            <q-tr>
              ...
              <td class="bg-teal-9 text-green-1">Price Change</td><td :class="getClass4Exp(p.row.price_change)">{{ showExpVal(p.row.price_change) }}</td>
            </q-tr>
            ...
            <q-tr>
              ...
              <td class="bg-teal-9 text-green-1">Percentage in Account</td><td :class="getClass4Exp(p.row.pct_of_account)">{{ showExpVal(p.row.pct_of_account) }} %</td>
            </q-tr>
            <q-tr>
              <td class="bg-teal-9 text-green-1">Cost Basis Per Share</td><td :class="getClass4Exp(p.row.cost_per_share)">{{ showExpVal(p.row.cost_per_share) }}</td>
              <td class="bg-teal-9 text-green-1">Cost Basis</td>          <td :class="getClass4Exp(p.row.cost_per_share)">{{ showExpVal(p.row.total_cost) }}</td>
            </q-tr>
            ...
            <q-tr v-if="p.row.low_52_week>0">
              <td class="bg-teal-9 text-green-1">52 Week Low </td><td :class="getClass4Exp(p.row.w52_low)">{{ showExpVal(p.row.low_52_week) }}</td>
              <td class="bg-teal-9 text-green-1">52 Week High</td><td :class="getClass4Exp(p.row.w52_high)">{{ showExpVal(p.row.high_52_week) }}</td>
            </q-tr>
          </table>
        </q-td>
      </q-tr>
...
<NumStrPad @set-num="updWeightPortfolio" />
</template>
<script setup>
import { ref } from 'vue'
...
//== main
const emit = defineEmits(['upd-weight-portfolio'])
console.log(`-ST-PortfolioPositions`)
emitter.on('open-PortfolioPositions',(x, y, z) => showPositions(x, y, z))

function col (idx) {
  const cols = [
    { required: false, label: '账号', align: 'right', name: 'account', field: 'account', headerClasses:'text-no-wrap', sortable: true },
    { required: true,  label: '账名', align: 'right', name: 'account_name', field: 'account_name', sortable: false, headerClasses:'text-no-wrap text-center' },
    ...
    { required: false, label: '日变', align: 'right', name: 'price_change', field: 'price_change', sortable: false, headerClasses:'text-no-wrap'  },
    ...
    { required: false, label: '总价', align: 'right', name: 'current_value', field: 'current_value', sortable: true, headerClasses:'text-no-wrap'  },
    ...
    { required: false, label: '进价', align: 'right', name: 'cost_per_share', field: 'cost_per_share', sortable: true, headerClasses:'text-no-wrap'  }
  ]
  return cols[idx]
}
function updWeightPortfolio (w, p) {
  const wkg = w * 0.453592
  console.log(`-fn-updWeightPortfolio wkg=${wkg} portfolio=${p}`)
  emit('upd-weight-portfolio', wkg, p)
}
...
function showExpVal (val) {
  // console.log(`-fn-showExpVal val=${val}`)
...
function getClass4Exp (val) {
  ...
  if (val == 'n/a' || val == '--' || val == null) return comClr
...
function updPortfValue () {
  title.value = selist.value[0].date + ": Portfolio Total Value: $" + totalVL.value
  // let zFund = selist.value.filter(a => a.account == 'Z71367818').map(p => p.current_val).reduce((x, y) => x + y, 0)
  // console.log(`-CK-ccPaymentInfo ccBalance=${ccBalance.value} zFund=${zFund} current_val=`, selist.value.filter(a => a.account == 'Z71367818'))
  // let tit = 'update weight and Portfolio Value
  // let msg = '<div>Fund - ccDue: ' + zFund.toFixed(2) + ' - ' + ccBalance.value + '</div>'
  let tit = '<div class="text-center text-h6">New Portfolio Value: </div><div class="text-red text-center text-bold text-h5">' + totalVL.value + '</div>'
  // let msg = "New Portfolio Value:" + totalVL.value
  emitter.emit('open-num-str-pad', tit, fidelTotal.value, weight.value)
}
function ccPaymentInfo () {
  let zFund = selist.value.filter(a => a.account == 'Z71367818').map(p => p.current_value).reduce((x, y) => parseFloat(x) + parseFloat(y), 0)
  console.log(`-CK-ccPaymentInfo ccBalance=${ccBalance.value} zFund=${zFund} current_value=`, selist.value.filter(a => a.account == 'Z71367818'))
...
function showPositions (da, portf, wgt) {
  console.log(`-fn-showPositions status=${da.status} portfolio=${portf} weight=${wgt}`, da)
  ...
  // selist.value = da.positions.sort((a, b) => b.account - a.account)
  const date = selist.value[0].date

  const total_value = selist.value.reduce((a, b) => a + b.current_value, 0)

  opened.value = true
  // emit('date-total-value', date, total_value)
  portfolio.value = portf
  // ppdiff.value = fmtcy(total_value - portf)
  // console.log("SELIST", selist.value.map(a => [a.symbol, a.accout, a.current_value]))
  const itemValues = selist.value.filter(p => p.account!='My-stocks').map(x => parseFloat(x.current_value))
  console.log(`-CK-%citemValues: ${itemValues.join('+')}`, "font-size:12px;font-weight:600;color:yellow")
  fidelTotal.value = itemValues.reduce((a, b) => a + b, 0)
  // console.log(`-CK-total_value=${fidelTotal.value}`)
  ppdiff.value = fmtcy(fidelTotal.value - portf)
  // console.log(`-CK-${portf} - ${fidelTotal} == ${ppdiff.value}`, fidelTotal.value)
  ...
  title.value = date + ' Portfolio Total Value: $' + fmtcy(portfolio.value) // + '    (' + total_items + ' Securities)'
  ...
  totalVL.value = fmtcy(fidelTotal.value)
  // portfolio.value = totalVL.value
