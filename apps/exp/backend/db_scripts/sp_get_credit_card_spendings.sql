...
WHERE ((purchasedon BETWEEN start_day AND end_day AND post_date IS NULL) 
      OR (post_date BETWEEN start_day AND end_day) 
      OR (sc.name = 'Refund' and sp.post_date = due_day))
      OR (sc.name = 'Trade in' and sp.post_date = due_day))
      OR (sc.name = 'Credit' and sp.post_date = due_day))
