      ...
      labels: {
        ...
        value: {
          ...
          formatter: function(value, ctx) {
            ...
            // return val != null ? val.toFixed(2) : ctx.dataset.data[ctx.dataIndex]
            // return '$' + fmtcy(val)
            return '$' + val
          },
          padding: 10,
...
const options = {
  onClick: (event, element, chart) =>{
    // console.warn(`cat`, event, element, chart)
    if (element.length === 0) {
      emitter.emit('close-chart', 'cata')
      return
    }
    const idx = element[0].index
    const cat = element[0].element.$datalabels[0].$context.dataset.cats[idx]
    const year = chart.data.year
    console.log(`cats-chart-config year=${year} cat=${cat}`)
    emitter.emit('open-ChartsProxy2', 'suba', cat)
  },
  plugins: {
    title: {
      display: true,
    ...
    tooltip: {
      ...
      callbacks: {
        label: (ctx) => {
          ...
          const note = val != null ? ' *' : ''
          // return ' Spending on ' + cat + ': $ ' + spending.toFixed(2) + note
          return ' Spending on ' + cat + ': $ ' + val
        }
      }
    },
  },
