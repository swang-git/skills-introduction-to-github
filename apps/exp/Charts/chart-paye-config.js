    ...
    datalabels: {
      ...
      labels: {
        ...
        cnts: {
          ...
          borderRadius: 50,
          formatter: (value, ctx) => {
            const cnt = ctx.dataset.cnts[ctx.dataIndex]
...
const options = {
  ...
  onClick:(event, element, chart) => {
    ...
    const payeId = chart.data.datasets[0].pyid[idx]
    const year = chart.data.year
    console.log(`-CK-open-ChartsProxy3', 'expp' year=${year} payeId=${payeId}`)
    emitter.emit('open-ChartsProxy3', 'expp', null, null, year, 0, payeId)
  },
  plugins: {
    title: {
      display: true,
