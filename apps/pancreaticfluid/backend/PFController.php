...
use App\Models\pfcheck\PfCheck;
...
class PFController extends Controller
{
  ...
  private function fmtLink($link, $swProp) {
	...
	public function getList($swProp) { Log::info("-fn-PFCheck-getList swProp=[$swProp]");
		$userId = Auth::user()->id; Log::info("-chk- userId=[$userId]");
		$dats = PfCheck::where([ ['status', 'A'], ['user_id', $userId] ])->select('id', 'user_id', 'datetime', 'vol', 'note')->orderBy('datetime', 'desc')->get();
		foreach($dats as $d) {
			$d->datetime = substr($d->datetime, 0, 16);
		}
		Log::info('pfcheck-field', $dats[0]->toArray());
	...
	public function upd(Request $da) { Log::info("upd-pfcheck id=$da->id vol=$da->vol swProp=[$da->swProp]");
		$nd = PfCheck::find($da->id);
		$dx = $da->toArray();
		foreach($dx as $k => $val) {
			if ($k == 'id' or $k == 'swProp') continue;
			$nd->$k = $val;
		}
		Log::info("-CK-upd pfcheck", $nd->toArray());
	...
	public function add(Request $d) { Log::info("-fn-PFController.add", $d->toArray());
		$nd = new PfCheck($d->toArray());
		// $swProp = $nd->swProp;
		// unset($nd->swProp);
		Log::info('new PfCheck to DB', [$nd]);
      ...
      Log::info('pfcheck/add', $nd->toArray());
			return ['row' => $nd, 'status' => "OK" ];
		} catch(Exception $ex) {
			return $ex->getMessage();
		}
