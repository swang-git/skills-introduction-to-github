...
class StatementNACController extends Controller {
  ...
  public function loadYearlyStatement($yyyymmdd) { Log::info("loadYearlyStatement $yyyymmdd");
    $docRoot = "/sites/webdata/docs/NAC/";
    $pdfnm = $docRoot . "${yyyymmdd}.pdf";
    // Log::info("loadYearlyStatement $pdfnm");
    $lines = $this->parsePDF($pdfnm);
    if (is_string($lines)) return ['info' => $lines, 'status' => 'NO_FILE'];
    $filename = "NAC_yearly_statement_$yyyymmdd";
    $this->writeToTempFile($filename, $lines);

    [$start, $acct] = $this->get_account_num(0, $lines);
    // Log::info("start=$start, acct=$acct");
    $da['account_num'] = $acct;
    [$start, $begin_date, $end_date] = $this->get_begin_end_date($start, $lines);
    Log::info("start=$start, begin_date=$begin_date");
    $da['begin_date'] = $begin_date;
    $da['end_date'] = $end_date;
    $da['typee'] = 'XXX_typend';

    $this->showData($da);
  }
  
  private function get_begin_end_date($start, $lines) {
    for ($i = $start; $i < count($lines) - $start; $i++) {
      $line = trim($lines[$i]);
      // $patt = '/\A\d{1,2}\/\d{1,2}\/\d{4} \- \d{1,2}\/\d{1,2}\d{4}$\z/';
      $patt = '/\A\d{1,2}\/\d{1,2}\/\d{4}\s+\-\s+\d{1,2}\/\d{1,2}\/\d{4}$\z/';
      if (preg_match($patt, $line)) {
        [$bdate, $edate] = explode(' - ', $line);
        return [$i++, $bdate, $edate];
      }
    }
  }

  private function get_account_num($start, $lines) {
    for ($i = $start; $i < count($lines) - $start; $i++) {
      $line = trim($lines[$i]);
      $patt = '/\A\d{10}$\z/';
      if (preg_match($patt, $line)) return [$i++, $line];
    }
  }


  private function showData($da) {
  ...
  //   return $chk;
  // }
  // public function loadYearlyStatement($ymon) { Log:info("loadYearlyStatements $ymon");
  //   $docRoot = "/sites/webdata/docs/NAC/";
