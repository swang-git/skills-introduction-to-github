...
console.log('-ST-MonthlyStatementsBOA')

emitter.on('open-MonthlyStatementsBOA', (x) => loadData(x))
emitter.on('close-MonthlyStatementsBOA', () => opened.value = 'none')
emitter.on('bankstatementloader-loadMonthlyStatementsBOA', (x) => setStmt(x))
...
function setStmt (da) {
  ...
  // console.log(`-CK-fn-setStmt loadMonthlyStatementsBOA chkinfo.chk=`, da.chkinfo.chk)
  // console.log(`-CK-fn-setStmt loadMonthlyStatementsBOA chkinfo.sav=`, da.chkinfo.sav)
  // console.log(`-CK-fn-setStmt loadMonthlyStatementsBOA savinfo=`, da.savinfo)
  console.log(`-CK-fn-setStmt loadMonthlyStatementsBOA da.status={$da.status} da.assets=`, da.assets, da)
...
function loadData (stmt) {
  ...
  month.value = x[1]
  ymon = x[0] + x[1]
  const path = ENV_DEV + '/bankstatementloader/loadMonthlyStatementsBOA/' + ymon
  gaxios(path)
}
