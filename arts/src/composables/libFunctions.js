...
export function libFunctions() {
  ...
  function desk () { return Platform.is.desktop }
  // function desk () {
  //   return (
  //     Platform.is.desktop &&
  ...
  // }
  function local() {
    const localhosts = /http:\/\/(prod|devx|192.168.|localhost|127.0.0.1|devx)/gi
  ...
  const SysAdmin = computed(() => {
    return golfUserType.value === 'SysAdmin' // || isSysAdminCookie
  })
  ...
  const KJsAdmin = computed(() => {
    return golfUserType.value === 'KJsAdmin'
  })
  const ALsAdmin = computed(() => {
    return golfUserType.value === 'ALsAdmin'
  })
