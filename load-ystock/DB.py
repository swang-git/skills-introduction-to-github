...
from Models import StockPrice, Portfolio, StockQuote
import sys, os, time
...
def loadQuantity():
    rec = session.query(Portfolio.asof_time, Portfolio.symbol, Portfolio.quantity)\
        .filter_by(status='A').order_by(Portfolio.asof_time.desc()).limit(100)
    ...
    for row in rec:
        ...
        try:
            if squan[symbol] > 0: continue ##squan[symbol] += quantity
